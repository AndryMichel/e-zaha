...
import {
    useGetEffectifCertificatsFonciers,
    useGetEffectifTitresCadastres
} from "@/services/api/region/indic-foncier-region.api";
import {
...
export function Foncier() {
    ...
    const {data: certificatsFonciersData, isLoading: isLoadingCertificats, isError: isErrorCertificats} =
        useGetEffectifCertificatsFonciers(apiParams);

    const {data: titresCadastresData, isLoading: isLoadingTitres, isError: isErrorTitres} =
        useGetEffectifTitresCadastres(apiParams);

    // État de chargement global
    const isLoading = isLoadingCertificats || isLoadingTitres;

    // État d'erreur global
    const hasError = isErrorCertificats || isErrorTitres;
    ...
    const safeGetTotalCertificatsFonciers = () => {
        if (hasMessageError(certificatsFonciersData)) {
            return null;
        }
        return certificatsFonciersData?.total_certificats_fonciers?.[0] ?? 0;
    };

    const safeGetMoyenneCertificatsFonciers = () => {
        if (hasMessageError(certificatsFonciersData)) {
            return null;
        }
        return certificatsFonciersData?.moyenne_certificats_fonciers_par_region?.[0] ?? 0;
    };

    const safeGetMinCertificatsFonciers = () => {
        if (hasMessageError(certificatsFonciersData)) {
            return null;
        }
        return certificatsFonciersData?.min_certificats_fonciers?.[0] ?? 0;
    };

    const safeGetMaxCertificatsFonciers = () => {
        if (hasMessageError(certificatsFonciersData)) {
            return null;
        }
        return certificatsFonciersData?.max_certificats_fonciers?.[0] ?? 0;
    };

    const safeGetTotalTitresCadastres = () => {
        if (hasMessageError(titresCadastresData)) {
            return null;
        }
        return titresCadastresData?.total_titres_cadastres?.[0] ?? 0;
    };

    const safeGetMoyenneTitresCadastres = () => {
        if (hasMessageError(titresCadastresData)) {
            return null;
        }
        return titresCadastresData?.moyenne_titres_cadastres_par_region?.[0] ?? 0;
    };

    const safeGetMinTitresCadastres = () => {
        if (hasMessageError(titresCadastresData)) {
            return null;
        }
        return titresCadastresData?.min_titres_cadastres?.[0] ?? 0;
    };

    const safeGetMaxTitresCadastres = () => {
        if (hasMessageError(titresCadastresData)) {
            return null;
        }
        return titresCadastresData?.max_titres_cadastres?.[0] ?? 0;
    };
    ...
    type DataType = (RegionCertificatsFonciersResponse['data'] & ResponseWithMessage) |
        (RegionTitresCadastresResponse['data'] & ResponseWithMessage) |
        null;
                            ...
                            <div className="w-full">
                                ...
                                {averageValue !== null && (
                                    <div className="p-3 bg-gray-50 rounded-lg text-center mb-4">
                                        <p className="text-sm text-gray-500">Moyenne par région</p>
                                        <p className="font-semibold">{averageValue.toLocaleString()}</p>
                                    </div>
                                )}
                                ...
                                {minValue !== null && maxValue !== null && (
                                    <div className="grid grid-cols-2 gap-4 mt-4">
                                        <div className="p-3 bg-gray-50 rounded-lg text-center">
                                            ...
                                            <p className="font-semibold">{minValue.toLocaleString()}</p>
                                        </div>
                                        <div className="p-3 bg-gray-50 rounded-lg text-center">
                                            <p className="text-sm text-gray-500">Maximum</p>
                                            <p className="font-semibold">{maxValue.toLocaleString()}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Carte pour les certificats fonciers */}
                {renderIndicatorCard(
                    "Effectif de certificats fonciers distribués",
                    certificatsFonciersData,
                    safeGetTotalCertificatsFonciers(),
                    safeGetMoyenneCertificatsFonciers(),
                    safeGetMinCertificatsFonciers(),
                    safeGetMaxCertificatsFonciers(),
                    "#8884d8",
                    "total de certificats fonciers distribués"
                )}

                {/* Carte pour les titres cadastres */}
                {renderIndicatorCard(
                    "Effectif de titres ou cadastres distribués",
                    titresCadastresData,
                    safeGetTotalTitresCadastres(),
                    safeGetMoyenneTitresCadastres(),
                    safeGetMinTitresCadastres(),
                    safeGetMaxTitresCadastres(),
                    "#82ca9d",
                    "total de titres cadastres distribués"
                )}
            </div>
        </div>
    );
