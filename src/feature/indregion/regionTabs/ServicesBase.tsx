...
import {
    ...
    useGetTauxAccesServiceEauBase
} from "@/services/api/region/indic-service-region.api";
import {
...
export function ServicesBase() {
    ...
    const {data: eauData, isLoading: isLoadingEau, isError: isErrorEau} =
        useGetTauxAccesServiceEauBase(apiParams);

    const {data: electriciteData, isLoading: isLoadingElectricite, isError: isErrorElectricite} =
        useGetTauxAccesElectricite(apiParams);

    // État de chargement global
    const isLoading = isLoadingEau || isLoadingElectricite;

    // État d'erreur global
    const hasError = isErrorEau || isErrorElectricite;
    ...
    const safeGetTauxAccesEau = () => {
        if (hasMessageError(eauData)) {
            return null;
        }
        return eauData?.taux_moyen_acces_eau?.[0] ?? 0;
    };

    const safeGetTauxAccesElectricite = () => {
        if (hasMessageError(electriciteData)) {
            return null;
        }
        return electriciteData?.taux_moyen_acces_electricite?.[0] ?? 0;
    };
    ...
    type DataType = (RegionAccesEauResponse['data'] & ResponseWithMessage) |
        (RegionAccesElectriciteResponse['data'] & ResponseWithMessage) |
        null;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Carte pour l'eau potable */}
                {renderIndicatorCard(
                    "Taux d'accès au service d'eau de base",
                    eauData,
                    safeGetTauxAccesEau(),
                    eauData?.taux_min_acces_eau?.[0] ?? null,
                    eauData?.taux_max_acces_eau?.[0] ?? null,
                    "#0088FE",
                    "des habitants ont accès à l'eau de base"
                )}

                {/* Carte pour l'électricité */}
                {renderIndicatorCard(
                    "Taux d'accès à l'électricité",
                    electriciteData,
                    safeGetTauxAccesElectricite(),
                    electriciteData?.taux_min_acces_electricite?.[0] ?? null,
                    electriciteData?.taux_max_acces_electricite?.[0] ?? null,
                    "#FFBB28",
