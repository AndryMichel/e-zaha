...
import {
    useGetTauxAbandonScolairePrimaire,
    useGetTauxBrutScolarisationPrimaire
} from "@/services/api/region/indic-education-region.api";
import {
...
export function Education() {
    ...
    const {data: scolarisationData, isLoading: isLoadingScolarisation, isError: isErrorScolarisation} =
        useGetTauxBrutScolarisationPrimaire(apiParams);

    const {data: abandonData, isLoading: isLoadingAbandon, isError: isErrorAbandon} =
        useGetTauxAbandonScolairePrimaire(apiParams);

    // État de chargement global
    const isLoading = isLoadingScolarisation || isLoadingAbandon;

    // État d'erreur global
    const hasError = isErrorScolarisation || isErrorAbandon;
    ...
    const safeGetTauxScolarisation = () => {
        if (hasMessageError(scolarisationData)) {
            return null;
        }
        return scolarisationData?.taux_moyen_scolarisation_primaire?.[0] ?? 0;
    };

    const safeGetTauxAbandon = () => {
        if (hasMessageError(abandonData)) {
            return null;
        }
        return abandonData?.taux_moyen_abandon_scolaire?.[0] ?? 0;
    };
    ...
    type DataType = (RegionTauxScolarisationPrimaireResponse['data'] & ResponseWithMessage) |
        (RegionTauxAbandonScolairePrimaireResponse['data'] & ResponseWithMessage) |
        null;
                                ...
                                {minValue !== null && maxValue !== null && (
                                    <div className="grid grid-cols-2 gap-4 mt-4">
                                        <div className="p-3 bg-gray-50 rounded-lg text-center">
                                            ...
                                            <p className="font-semibold">{minValue}{unit}</p>
                                        </div>
                                        <div className="p-3 bg-gray-50 rounded-lg text-center">
                                            <p className="text-sm text-gray-500">Maximum</p>
                                            <p className="font-semibold">{maxValue}{unit}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Carte pour le taux de scolarisation */}
                {renderIndicatorCard(
                    "Taux brut de scolarisation au primaire",
                    scolarisationData,
                    safeGetTauxScolarisation(),
                    scolarisationData?.taux_min_scolarisation_primaire?.[0] ?? null,
                    scolarisationData?.taux_max_scolarisation_primaire?.[0] ?? null,
                    "#2E7D32",
                    "taux brut de scolarisation au niveau primaire"
                )}

                {/* Carte pour le taux d'abandon */}
                {renderIndicatorCard(
                    "Taux d'abandon scolaire au niveau primaire",
                    abandonData,
                    safeGetTauxAbandon(),
                    abandonData?.taux_min_abandon?.[0] ?? null,
                    abandonData?.taux_max_abandon?.[0] ?? null,
                    "#C62828",
                    "taux d'abandon scolaire au niveau primaire"
                )}
            </div>
