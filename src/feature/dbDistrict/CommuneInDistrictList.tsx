...
import {CommuneInDistrict, useGetCommuneInDistrict} from "@/services/api/commune/get-commune-liste-trie.api";

interface CommuneInDistrictListProps {
...
export function CommuneInDistrictList({
                                          selectedDistrictCode = null,
                                          selectedDistrictName = null,
                                          onResetDistrict = () => {
                                          },
                                      }: CommuneInDistrictListProps) {
    const router = useRouter();
    const {data: session} = useSession();
    const userRole = session?.user?.role;

    const getDistrictCode = (): string => {
        if (selectedDistrictCode) return selectedDistrictCode;
        if (userRole === "ODDL" || userRole === "administrateur") return "all";

        // For both district and regional roles, return the district code
        if (userRole === "district" || userRole === "regional") {
            const rawCode = session?.user?.location?.district_code;
    ...
    const [sortBy, setSortBy] = useState("commune_name");
    ...
    const {communes, total, currentPage, totalPages, isLoading, isError, mutate} =
        useGetCommuneInDistrict(districtCode, page, limit, search, sortBy, sortOrder, regionCode);
    ...
    const getDistrictTitle = () => {
        if (selectedDistrictCode && selectedDistrictName) {
            return `Communes du district ${selectedDistrictName}`;
        }
        if (districtCode === "all") return "Toutes les communes";
        if (userRole === "regional")
            return `Communes de la région ${session?.user?.location?.region || ''}`;
        return `Communes du district ${session?.user?.location?.district || ''}`;
    };

    const handleStatusToggle = (commune: CommuneInDistrict) => {
        setSelectedCommune({
    ...
    const navigateToDbCommune = (commune: CommuneInDistrict) => {
    ...
    const districtTitle = getDistrictTitle();

    return (
        <div className="space-y-4">
            {selectedDistrictCode &&
                (userRole === "ODDL" || userRole === "administrateur" || userRole === "regional") && (
                    <Button
                        variant="outline"
                        size="sm"
            ...
            {(userRole === "ODDL" || userRole === "administrateur")
                ...
                <div className="flex items-center gap-2">
                    <div className="flex items-center text-xs text-gray-600">
                        ...
                        >
                            Nom {sortBy === "commune_name" && (sortOrder === "asc" ? "↑" : "↓")}
                        </Button>
                        <Button
                            variant="ghost"
                            size="sm"
                        ...
                        >
                            District {sortBy === "district_code" && (sortOrder === "asc" ? "↑" : "↓")}
                        </Button>
                    </div>
